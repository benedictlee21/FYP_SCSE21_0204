...
class MSN(nn.Module):
    ...
    def forward(self, x):
        ...
        xx1 = xx[:, 0:3, :].transpose(1, 2).contiguous()

        resampled_idx = MDS_module.minimum_density_sample(xx1, out1.shape[1], mean_mst_dis)
   
        # after MDS
        xx2 = MDS_module.gather_operation(xx, resampled_idx)
        delta = self.res(xx2)
        xx2 = xx2[:, 0:3, :]
        out2 = (xx2 + delta).transpose(2,1).contiguous()
        xx2 = xx2.transpose(1, 2).contiguous()
