...
class PatchDection(nn.Module):
    def __init__(self, numpoint, divide_ratio=1):
        super().__init__()
        self.pointnet_sa_module = PointnetSAModuleMSG(npoint=int(numpoint/8), radii=[0.1, 0.2, 0.4], nsamples=[16, 32, 128],
...
class CascadeGenerator(nn.Module):
    ...
    def forward(self, input_batch, mean_feature_batch):
         '''
         :param input: B * C * N
         :param gt: B * N * C
         :return: G_loss, D_loss
         '''
         features_partial_pt = self.encoder(input_batch)
         coarse, fine = self.decoder(features_partial_pt, input_batch, self.step_ratio, mean_feature=mean_feature_batch)
