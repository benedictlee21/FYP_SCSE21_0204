...
from models.cascade_model import CascadeGenerator
...
def test(args):
    ...
    for epoch in epochs:
        ...
        if args.model_name == 'cascade':
            ...
            net = PCN(num_coarse=1024, num_fine=args.num_points)
        ...
        pcd_file = h5py.File(os.path.join(args.model_dir, 'outputs', args.model_name, '%s_pcds.h5' % epoch.split('.')[0]), 'w')
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader_test):
                ...
                if args.model_name == 'topnet':
                    output = net(inputs)
                elif args.model_name == 'cascade':
                    _, output = net(inputs, mean_feature)
                elif args.model_name == 'msn':
                    _, output, _ = net(inputs)
                elif args.model_name == 'pcn':
                    _, output = net(inputs)
                else:
                    raise NotImplementedError
