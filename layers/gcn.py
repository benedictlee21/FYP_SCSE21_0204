...
class TreeGCN(nn.Module):
    def __init__(self, batch, depth, features, degrees, support=10, node=1, upsample=False, activation=True,args=None):
        self.batch = batch
        self.depth = depth
    ...
    def forward(self, tree):
        ...
        self.batch = tree[0].shape[0] # TODO to change the self.batch at init # to enable it for multi-GPU
        root = 0
        # ancestor term
        ...
        branch = 0
        if self.upsample:
            branch = tree[-1].unsqueeze(2) @ self.W_branch ### TODO figure out what it does
