...
# Compute the chamfer distance for small batches.
...
def retrieve_ours_pcs(input_dir, with_input = True, with_gt = True):
    ...
    # For each shape being evaluated.
    for idx in index_ls:
    
        # Read in the partial shape.
        ...
        # If the evaluation uses the ground truth.
        if with_gt: 
        ...
        # If the evaluation uses the completed shape.
        if with_input:
...
def eval_completion_without_gt(input_dir):
    ...
    # Retrieve the partial and completed shapes from the specified directory.
    ...
    # Save the partial and ground truth for each shape as a numpy array into a list.
    for idx in index_ls:
    ...
    # Compute the UCD and UHD metrics.
...
if __name__ == '__main__':
    ...
    # Read in the argument for the path of the shapes to be evaluated.
