...
class BenchmarkDataset(data.Dataset):
    ...
    def __init__(self, root, npoints=2500, uniform=False, classification=True, class_choice=None,n_samples_train=0):
        ...
        if n_samples_train == 0:
            ...
            np.random.seed(0)
            # TODO hard code for chair
            for item in self.cat:
                for fn in self.meta[item]:
                    self.datapath.append((item, fn[0], fn[1], fn[2]))
            perm_list = np.random.permutation(len(self.datapath)).tolist()[:n_samples_train]
            self.datapath = []
            for item in self.cat:
    ...
    def __getitem__(self, index):
        ...
        # print(self.i, 'num points,',point_set.shape[0])
        # self.i+=1
        if self.uniform:
            ...
            # NOTE: changed to permutation #TODO seed should be in the init
        ...
        if self.classification:
            return point_set, cls #fn[1] # NOTE: previously it is cls, not pathname
        else:
